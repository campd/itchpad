...
var Itchpad = Class({
  ...
  initialize: function(window, project, toolbox) {
    ...
    this.liveTree = new CollectionTree(this.document, {
      nodeVisible: this.nodeVisible.bind(this),
      nodeFormatter: this.formatNode.bind(this)
    });
  ...
  setPage: function(page) {
    ...
    this.liveTree.setCollection(page);
  },
  ...
  setTarget: function(target, own=false) {
    ...
    if (!target) {
      this.document.querySelector("#sources-tabs").selectedIndex = 0;
      this.document.querySelector("#sources-tabs > tabs").hidden = true;
    } else {
      this.document.querySelector("#sources-tabs > tabs").hidden = false;
    }
...
var CollectionTree = Class({
  ...
  setCollection: function(coll) {
    if (this.coll) {
      this.coll.off("store-added", this.addModel);
      this.coll.off("store-removed", this.removeModel);
    ...
    this.coll = coll;
    if (this.coll) {
      this.coll.on("store-added", this.addModel);
      this.coll.on("store-removed", this.removeModel);
      for (let store of this.coll.allStores()) {
        this.addModel(store)
      }
    }
...
exports.forProject = function(proj) {
