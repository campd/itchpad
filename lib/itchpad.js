...
const { ScratchStore } = require("stores/local");
...
var Itchpad = Class({
  ...
  initialize: function(window, toolbox) {
    ...
    this.project.on("store-added", this.onProjectStoreAdded.bind(this));
    this.project.on("store-removed", this.onProjectStoreRemoved.bind(this));
    try {
      this.project.loadPref();
    } catch(ex) {
      console.error(ex);
    }
