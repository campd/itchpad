...
var Itchpad = Class({
  ...
  initialize: function(document, toolbox) {
    ...
    this.editorPane.on("editor-activated", this.onEditorActivated);
    ...
    this.addRemoteStore(this.styles);
    this.loadPlugins();
  },
  ...
  onNodeSelection: function(resource) {
    ...
    let editor = this.editorPane.open(pair, resource);
  ...
  addScratchStore: function(store) {
    ...
    this.resourceMap.addLocalStore(store, { noAutoPair: true });
  },
