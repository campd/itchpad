...
var Projects = Class({
    ...
    return task.spawn(function() {
      ...
      while (true) {
        ...
        try {
          ...
          let projectData = JSON.parse(gDecoder.decode(bytes));
          for (let dir of projectData.directories) {
            dir = OS.Path.normalize(dir);
            if (!projs.has(dir)) {
              projs.set(dir, []);
            }
            projs.get(dir).push({ path: entry.path, data: projectData });
          }
        } catch(ex) {
          console.error(ex);
        }
  ...
  forDir: function(path) {
    ...
    return this.init().then(() => {
      ...
      let projectPath = OS.Path.join(this._projectStorageDir, basename + ".fx-project");
  ...
  forManifest: function(path) {
    ...
    return this.forDir(dir).then(project => {
      return project.setManifest(path).then(() => project.save()).then(() => project);
    });
  },
  ...
  forPath: function(path) {
    ...
    return OS.File.stat(path).then(info => {
      ...
      if (OS.Path.basename(path) === "manifest.webapp") {
        return this.forManifest(path);
      }

      return OS.File.read(path).then(bytes => {
        let json = JSON.parse(gDecoder.decode(bytes));
        ...
        throw new Error("Don't understand this file type.");
      });
    });
  }
});
