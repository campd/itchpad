...
var PairEditor = Class({
  ...
  initialize: function(document, pair, selectedResource) {
    ...
    this.source = selectedResource === this.live ? "live" : "project";
  ...
  onPairChanged: function() {
    this.onSourceChange("project");
    this.onSourceChange("live");
  },

  onSourceChange: function(source) {
    if (this.pair[source] === this[source]) {
      return;
    }

    this[source] = this.pair[source];

    if (this.editors[source]) {
      // Reload from the new source
      this.editors[source].load(this.pair[source]);
    }
  },
  ...
  // Source can be either "project" or "live"
  selectSource: function(source) {
    let source = (source === "project") ? "project" : "live";
  ...
  _resourceFor: function(operation) {
    ...
    let other = (this.source === "project") ? this.live : this.project;
